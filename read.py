...
def _parse_3ds_header(header_raw, header_override):
    ...
    header_entries = header_raw.strip().splitlines()
   # print(header_entries,"he")

    # Convert the strings to a dictionary.
    raw_dict = dict()
    for entry in header_entries:
           if entry.strip() == ":HEADER_END:":  # Skip the :HEADER_END: marker
              # print(f"Skipping marker: {entry}")
               continue
           if "=" not in entry:  # Skip lines that are not valid key-value pairs
...
def _parse_sxm_header(header_raw):
    ...
    for key in entries_to_be_split:
        try:
            ...
            print(f"Warning: '{key}' not found in header. Skipping this entry.")
    
    # Convert to float if key exists
